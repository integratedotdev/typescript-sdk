...
describe("YouTube Integration", () => {
  ...
  test("includes expected tools", () => {
    ...
    expect(integration.tools).toContain("youtube_create_playlist");
    expect(integration.tools).toContain("youtube_subscribe");
  });
  ...
  test("reads credentials from environment variables", () => {
    const originalId = process.env.YOUTUBE_CLIENT_ID;
    const originalSecret = process.env.YOUTUBE_CLIENT_SECRET;

    process.env.YOUTUBE_CLIENT_ID = "env-client-id";
    process.env.YOUTUBE_CLIENT_SECRET = "env-client-secret";
    ...
    if (originalId === undefined) delete process.env.YOUTUBE_CLIENT_ID;
    else process.env.YOUTUBE_CLIENT_ID = originalId;
    if (originalSecret === undefined) delete process.env.YOUTUBE_CLIENT_SECRET;
    else process.env.YOUTUBE_CLIENT_SECRET = originalSecret;
  });
