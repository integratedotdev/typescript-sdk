...
describe("Server-Side toNextJsHandler", () => {
  ...
  it("should resolve DB-backed provider tokens for code mode only when callback API key matches", async () => {
    const callbackArgs: Array<{ provider?: string; email?: string; context?: any }> = [];
      ...
      if (url.includes("/api/v1/mcp")) {
        ...
        return {
          ...
          json: async () => ({
            ...
            id: 1,
            result: {
              content: [{ type: "text", text: "context auth result" }],
            },
          }),
          text: async () => "error text",
        } as Response;
    ...
    const { client } = createMCPServer({
      ...
      getProviderToken: async (provider, email, context) => {
        callbackArgs.push({ provider, email, context });
        return { accessToken: "db-token-123" } as any;
      },
    });
    ...
    expect(response.status).toBe(200);
    expect(data.content[0].text).toBe("context auth result");
    expect(callbackArgs.some((entry) =>
      entry.provider === "github" &&
      entry.email === undefined &&
      entry.context?.userId === "user_123"
    )).toBe(true);
    expect(downstreamAuthorization).toBe("Bearer db-token-123");
  });

  it("should skip code mode context fallback when callback API key is missing or invalid", async () => {
    const callbackArgs: Array<{ provider?: string; email?: string; context?: any }> = [];
    let downstreamAuthorization: string | undefined;
      ...
      if (url.includes("/api/v1/mcp")) {
        ...
        return {
          ...
          json: async () => ({
            ...
            id: 1,
            result: {
              content: [{ type: "text", text: "unauthenticated result" }],
            },
          }),
          text: async () => "error text",
        } as Response;
    ...
    const request = {
      ...
      headers: {
        get: (key: string) => {
          if (key === "x-integrate-code-mode") return "1";
          if (key === "x-integrate-context") return JSON.stringify({ userId: "victim" });
          if (key === "x-integrate-api-key") return "wrong-key";
          return null;
        },
      },
      json: async () => ({
    ...
    expect(response.status).toBe(200);
    expect(data.content[0].text).toBe("unauthenticated result");
    expect(callbackArgs.some((entry) => entry.context?.userId === "victim")).toBe(false);
    expect(downstreamAuthorization).toBeUndefined();
  });

  it("should preserve session Authorization for code mode when provider tokens are also present", async () => {
    let downstreamAuthorization: string | undefined;
      ...
      if (url.includes("/api/v1/mcp")) {
        ...
        return {
          ...
          json: async () => ({
            ...
            id: 1,
            result: {
              content: [{ type: "text", text: "session auth result" }],
            },
          }),
          text: async () => "error text",
        } as Response;
    ...
    const request = {
      ...
      headers: {
        get: (key: string) => {
          if (key === "authorization") return "Bearer sess_123";
          if (key === "x-integrate-code-mode") return "1";
          if (key === "x-integrate-tokens") return JSON.stringify({ github: "ghp_fallback" });
          return null;
        },
      },
      json: async () => ({
