...
- **Provider Extraction**: Extracts provider from tool name (e.g., `gmail_send_email` → `gmail`)
...
- Extracts provider from tool name: `gmail_send_email` → `gmail`
...
- Validates status transitions:
  - Pause: Only allows `active` → `paused`
  - Resume: Only allows `paused` → `active`
