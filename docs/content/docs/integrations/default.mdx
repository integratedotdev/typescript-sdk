...
Get a list of all integrations configured on your SDK client. By default, this is a local-only operation that doesn't make any server calls. Optionally, you can fetch full tool metadata for all configured integrations:

```typescript
// Fast: Get configured integrations (local only, no server calls)
