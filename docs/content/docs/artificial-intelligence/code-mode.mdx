...
export const { client: serverClient } = createMCPServer({
  ...
  codeMode: {
    publicUrl: process.env.INTEGRATE_PUBLIC_URL, // e.g. "https://myapp.com"
  },
});
```
...
```bash
INTEGRATE_PUBLIC_URL=https://myapp.com
```

<Callout type="warn">
Code Mode will return a clear error if `publicUrl` is not set. The sandbox needs it to call back into your `/api/integrate/mcp` route.
</Callout>
...
createMCPServer({
  // ...
  codeMode: {
    publicUrl: process.env.INTEGRATE_PUBLIC_URL,
