...
List integrations configured on this SDK client. By default, this is a local-only operation that doesn't make any server calls. Optionally, you can fetch full tool metadata (descriptions, input schemas) for all configured integrations.
...
**Example (local only - fast):**

```typescript
// Get all configured integrations (no server call)
...
**Note:** 
- Without `includeToolMetadata`, this method returns only local configuration data (no server calls).
